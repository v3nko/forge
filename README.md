...
- [`forge/base`](images/base/README.md): shared Ubuntu-based SSH workstation with a non-root `dev` user, persistent home behavior, stable shell tooling, Git, SSH, sudo, Python, Node.js/npm, Homebrew (`brew`), Antigravity CLI (`agy`), OpenCode (`opencode`), Crush (`crush`), and RTK (`rtk`).
